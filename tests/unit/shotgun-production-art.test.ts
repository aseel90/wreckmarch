...
describe('WS14-C Shotgun production art assets',()=>{
 ...
 it('stays art-only and unregistered',()=>{expect(SHOTGUN_PRODUCTION_ART.activation.playableOnMain).toBe(false);expect(read('src/characters/character-registry.js')).not.toMatch(/shotgun/i);expect(read('index.html')).not.toContain('shotgun-production-art')});
});