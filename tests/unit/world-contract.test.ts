...
describe('R2 canonical world contract', () => {
  it('promotes the selected 9600 world to the single production geometry', () => {
    expect(CURRENT_PRODUCTION_WORLD).toMatchObject({ id: 'production-9600-v1', width: 9600, height: 9600, role: 'selected-production-world' });
  ...
  it('uses 1200-unit technical sectors and an 8x8 production grid', () => {
    expect(WORLD_SECTOR_POLICY).toMatchObject({ sectorSize: 1200, activeRadius: 1, maxActiveSectors: 9 });
    expect(getWorldSectorGrid(CURRENT_PRODUCTION_WORLD)).toEqual({
      worldId: 'production-9600-v1', sectorSize: 1200, columns: 8, rows: 8, sectorCount: 64
    });
  ...
  it('clamps positions and never activates more than a 3x3 neighborhood', () => {
    expect(getWorldSectorForPosition(-50, -90, CURRENT_PRODUCTION_WORLD)).toMatchObject({ key: '0:0', column: 0, row: 0 });
    expect(getWorldSectorForPosition(9600, 9600, CURRENT_PRODUCTION_WORLD)).toMatchObject({ key: '7:7', column: 7, row: 7 });
    const center = getActiveWorldSectors(4800, 4800, CURRENT_PRODUCTION_WORLD);
    const edge = getActiveWorldSectors(0, 0, CURRENT_PRODUCTION_WORLD);
