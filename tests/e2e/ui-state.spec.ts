...
test('upgrade overlay suppresses gameplay HUD and restores it after selection UI closes', async ({ page }) => {
  ...
  const opened = await page.evaluate(async () => {
    ...
    return {
      ...
      upgradeSceneActive: game.scene.isActive('UpgradeSceneV4'),
      titleVisible: scene.titleText.visible,
      xpVisible: scene.xpBg.visible,
      joystickVisible: scene.joyBase.visible,
      railVisible: rail?.visible,
      hudState: document.documentElement.dataset.wreckmarchGameplayHud
    };
  });

  expect(opened).toMatchObject({
    upgradeOpen: true,
    upgradeSceneActive: true,
    titleVisible: false,
    xpVisible: false,
...
test('end-run overlay uses the live landscape viewport instead of legacy portrait constants', async ({ page }) => {
  ...
  const result = await page.evaluate(() => {
    ...
    return {
      ...
      overlay: {
        x: layout.overlay.x, y: layout.overlay.y,
        width: layout.overlay.displayWidth, height: layout.overlay.displayHeight,
        scrollFactorX: layout.overlay.scrollFactorX, scrollFactorY: layout.overlay.scrollFactorY
      },
      button: { x: layout.btn.x, y: layout.btn.y },
      titleVisible: scene.titleText.visible,
      hudState: document.documentElement.dataset.wreckmarchGameplayHud,
