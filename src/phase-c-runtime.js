...
function createUpgradePool(scene) {
  return [
    {
      id: 'heavy-rivets', category: 'HERO', title: 'HEAVY RIVETS', desc: '+20% Rivet Gun damage.', weight: 1.25,
      available: () => upgradeLevel(scene, 'heavy-rivets') < 5,
      apply: () => { bumpUpgrade(scene, 'heavy-rivets'); scene.primaryWeapon.damage *= 1.2; scene.damage = scene.primaryWeapon.damage; }
    },
    {
...
function makeCard(scene, y, upgrade, index) {
  ...
  const bg = scene.add.rectangle(0, 0, 430, 142, 0x151b22, .98)
    .setStrokeStyle(2, index === 0 ? 0xd0a862 : 0x56636f, .95)
    .setInteractive({ useHandCursor: true });
  const category = scene.add.text(-190, -50, upgrade.category, {
    fontFamily: 'Arial Black, Arial', fontSize: '11px', color: upgrade.category === 'FORTRESS' ? '#55d8e6' : '#d8b06d'
  }).setOrigin(0, .5);
  const title = scene.add.text(-190, -20, upgrade.title, {
    fontFamily: 'Arial Black, Arial', fontSize: '20px', color: '#f0f2f4'
  }).setOrigin(0, .5);
  const desc = scene.add.text(-190, 18, upgrade.desc, {
    fontFamily: 'Arial, sans-serif', fontSize: '14px', color: '#aeb8c2', wordWrap: { width: 360 }
  }).setOrigin(0, 0);
...
function installUpgradeCards(scene) {
  ...
  scene.openUpgradeCards = function() {
    ...
    const label = this.add.text(W / 2, 150, `LEVEL ${this.level}`, {
      fontFamily: 'Arial Black, Arial', fontSize: '14px', color: '#61d9e6'
    }).setOrigin(.5).setDepth(4201).setScrollFactor(0);
    const title = this.add.text(W / 2, 186, 'CHOOSE AN UPGRADE', {
      fontFamily: 'Arial Black, Arial', fontSize: '25px', color: '#f0d09b'
    }).setOrigin(.5).setDepth(4201).setScrollFactor(0);
    this.upgradeUi = [shade, label, title];
    [300, 465, 630].forEach((y, i) => {
      if (choices[i]) this.upgradeUi.push(makeCard(this, y, choices[i], i));
    });
  };
