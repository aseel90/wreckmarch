...
function selfTest(s){if(new URLSearchParams(location.search).get('autotest')!=='1')return;
  const presentation=s.__characterPresentationD1,allHdCards=CARD_IDS.every(id=>s.textures.get('c3-atlas')?.has?.(CARD_FRAME(id))),rarities=new Set(s.__d1RarityStyles||[]),terrain=s.worldSectorTerrain?.getDiagnostics?.(),truck=s.__d1Wrecks?.find(o=>o.__vehicleKind==='truck'),sedan=s.__d1Wrecks?.find(o=>o.__vehicleKind==='sedan');
  const probe=s.add.image(-9999,-9999,'c3-atlas',CARD_FRAME('overclock'));fit(probe,112,96);
  const checks={characterPresentation:presentation?.characterId===s.characterDefinition?.id&&presentation?.ok===true,...(presentation?.checks||{}),premiumCards:allHdCards&&probe.texture?.key==='c3-atlas'&&probe.displayWidth>=80&&s.textures.exists('upgrade-icon-piercing-rivets')&&s.textures.exists('upgrade-icon-ricochet')&&s.textures.exists('upgrade-icon-shrapnel-impact')&&s.textures.exists('upgrade-icon-critical-rivet')&&s.textures.exists('upgrade-icon-explosive-rivet')&&s.textures.exists('upgrade-icon-triple-riveter')&&s.__upgradeCardArtReady===true,cardPresentation:s.__upgradeCardPresentationVersion==='u5-before-after-v3'&&Array.isArray(s.__upgradeCardVisualHierarchy)&&s.__upgradeCardVisualHierarchy.join('>')==='ART>NAME>RARITY>LEVEL>PREVIEW>DESCRIPTION',cardPreview:s.__upgradeCardPreviewVersion==='u5-before-after-v1',rarityCards:['COMMON','RARE','EPIC','LEGENDARY'].every(r=>rarities.has(r)),roadsVisible:(terrain?.activeRoadObjectCount||0)>0&&terrain?.visibleRoadObjectCount===terrain?.activeRoadObjectCount&&terrain?.fullMapTerrainAllocated===false&&(terrain?.activeSectorCount||0)<=9,vehicleScale:!!truck&&!!sedan&&truck.displayWidth>=330&&sedan.displayWidth>=225&&truck.displayWidth>sedan.displayWidth};probe.destroy();
