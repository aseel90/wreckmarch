...
import { loadRunnerLocomotionArt } from './characters/runner-locomotion-art.js?v=2';
...
function installVehicleScale(s){hideLegacyWrecks(s);s.__d1Wrecks?.forEach?.(o=>o?.destroy?.());s.__d1Wrecks=[];const P=[['sedan',450,780,-.12,false],['truck',1720,720,.10,true],['overturned',700,1460,.18,false],['van',1580,1510,-.12,false],['sedan',1880,1180,.21,true],['van',330,1270,.08,true],['truck',1320,390,-.07,false],['overturned',1960,1880,-.18,true]];P.forEach(v=>addWreck(s,...v));s.vehicleScaleProfile=VEHICLE_PROFILE;s.__d1VehicleScale=true}
...
function installPremiumCards(s){
  ...
  up.card=function(x,y,w,h,u,i){
    const category=COLORS[u.category]||COLORS.HERO,rarity=CARD_RARITY[u.id]||'COMMON',style=RARITY_STYLE[rarity],rank=style.rank,g=this.add.container(x,y);
    const shadow=this.add.rectangle(8,11,w,h,0,.46),glow=this.add.rectangle(0,0,w+8,h+8,style.glow,rank>=2?.10:rank===1?.055:.02),bg=this.add.rectangle(0,0,w,h,0x141a21,.997).setStrokeStyle(rank>=3?4:rank>=1?3:2,style.frame,.92),inner=this.add.rectangle(0,0,w-10,h-10,0,0).setStrokeStyle(1,style.frame,rank>=1?.42:.14),header=this.add.rectangle(0,-h/2+27,w-4,50,0x0b1016,.98),strip=this.add.rectangle(0,-h/2+5,w,10,style.frame,.98);
    const categoryText=this.add.text(-w/2+16,-h/2+28,u.category,{fontFamily:'Arial Black,Arial',fontSize:'10px',color:Phaser.Display.Color.IntegerToColor(category).rgba}).setOrigin(0,.5),rarityText=this.add.text(w/2-16,-h/2+28,style.label,{fontFamily:'Arial Black,Arial',fontSize:rank>=3?'9px':'8px',color:Phaser.Display.Color.IntegerToColor(style.frame).rgba,letterSpacing:1}).setOrigin(1,.5);
    const artH=Math.min(154,h*.39),artY=-h*.18,artBg=this.add.rectangle(0,artY,w-34,artH,0x0a1015,.96).setStrokeStyle(2,style.frame,rank>=2?.48:.28),iconPlate=this.add.circle(0,artY,Math.min(58,artH*.39),0x111a20,1).setStrokeStyle(3,category,.42),art=this.add.image(0,artY,'c3-atlas',CARD_FRAME(u.id));fit(art,Math.min(116,w*.43),Math.min(102,artH*.68));
    const title=this.add.text(0,h*.105,u.title,{fontFamily:'Arial Black,Arial',fontSize:`${Math.max(16,Math.min(21,w/13.6))}px`,color:'#f4f6f7',align:'center',wordWrap:{width:w-28}}).setOrigin(.5),desc=this.add.text(0,h*.225,u.desc,{fontFamily:'Arial',fontSize:'12px',color:'#c1c9d0',align:'center',wordWrap:{width:w-34},lineSpacing:3}).setOrigin(.5,0),lv=this.gameScene?.upgradeLevels?.[u.id]||0,footerLabel=lv?`LV ${lv}  →  ${lv+1}`:'NEW UPGRADE',footer=this.add.text(0,h/2-23,footerLabel,{fontFamily:'Arial Black,Arial',fontSize:'9px',color:Phaser.Display.Color.IntegerToColor(style.frame).rgba}).setOrigin(.5);
    const accents=[];if(rank>=1){accents.push(this.add.rectangle(-w/2+5,0,3,h*.52,style.frame,.34));accents.push(this.add.rectangle(w/2-5,0,3,h*.52,style.frame,.34))}if(rank>=2){accents.push(this.add.rectangle(-w/2+8,-h/2+9,28,3,style.frame,.82));accents.push(this.add.rectangle(w/2-8,-h/2+9,28,3,style.frame,.82));accents.push(this.add.rectangle(0,h/2-7,w*.42,3,style.frame,.65))}if(rank>=3){[[-1,-1],[1,-1],[-1,1],[1,1]].forEach(([sx,sy])=>accents.push(this.add.circle(sx*(w/2-13),sy*(h/2-13),4,0xf7d58a,1).setStrokeStyle(2,0xd98446,.9)));accents.push(this.add.rectangle(0,-h/2+12,w*.54,3,0xf7d58a,.82))}
    const hit=this.add.zone(0,0,w,h).setInteractive({useHandCursor:true});hit.on('pointerover',()=>{this.selectedIndex=i;this.refresh()});hit.on('pointerdown',(_p,_x,_y,e)=>{e?.stopPropagation?.();this.choose(i)});g.add([shadow,glow,bg,inner,header,strip,categoryText,rarityText,artBg,iconPlate,art,title,desc,footer,...accents,hit]);this.cards.push({g,bg,inner,strip,art,glow,rarityText,rarity,style,a:style.frame});
  };
  up.refresh=function(){this.cards.forEach((v,i)=>{const selected=i===this.selectedIndex,rank=v.style.rank;v.g.setScale(selected?1.028:1);v.bg.setStrokeStyle(selected?(rank>=3?5:4):(rank>=3?4:rank>=1?3:2),v.style.frame,selected?1:.88);v.inner.setAlpha(selected?1:(rank>=1?.72:.4));v.strip.setAlpha(selected?1:.84);v.glow.setAlpha(selected?(rank>=2?.20:.10):(rank>=2?.10:rank===1?.055:.02));v.art.setAlpha(selected?1:.94);v.rarityText.setAlpha(selected?1:.86)})};
  s.__d1PremiumCards=true;s.__d1CardRarity={...CARD_RARITY};s.__d1CardArtSource='c3-atlas-icons';
}

function selfTest(s){if(new URLSearchParams(location.search).get('autotest')!=='1')return;const runFrames=s.anims.get(s.characterDefinition?.animations?.run?.key)?.frames?.length||0,hdSheet=s.textures.get('c5-upgrade-sheet'),allHdCards=CARD_IDS.every(id=>s.textures.get('c3-atlas')?.has?.(CARD_FRAME(id))),rarities=new Set(Object.values(s.__d1CardRarity||{})),roads=(s.__e0FastRoadSegments||[]).filter(o=>o?.active!==false),near=roads.some(o=>Phaser.Math.Distance.Between(o.x,o.y,WORLD_W/2,WORLD_H/2)<180),truck=s.__d1Wrecks?.find(o=>o.__vehicleKind==='truck'),sedan=s.__d1Wrecks?.find(o=>o.__vehicleKind==='sedan');const probe=s.add.image(-9999,-9999,'c3-atlas',CARD_FRAME('overclock'));fit(probe,112,96);const checks={animatedLegs:runFrames>=6&&s.__d1AnimatedRunner===true,mechanicalArm:!!s.__d1MechanicalArm&&s.weaponModule===s.weaponV3Gun&&GUN_FRAMES.includes(s.weaponV3Gun.frame?.name),noHandSprites:[s.weaponV3ArmA,s.weaponV3ArmB,s.weaponV3HandA,s.weaponV3HandB,s.weaponArm,s.weaponRig].every(o=>!o||o.visible===false),premiumCards:allHdCards&&probe.texture?.key==='c3-atlas'&&probe.displayWidth>=80,rarityCards:['COMMON','RARE','EPIC','LEGENDARY'].every(r=>rarities.has(r)),roadsVisible:roads.length>200&&near&&roads.every(o=>o.visible&&o.alpha>.95&&o.displayHeight>=145&&o.__terrainSystemObject),vehicleScale:!!truck&&!!sedan&&truck.displayWidth>=330&&sedan.displayWidth>=225&&truck.displayWidth>sedan.displayWidth,characterSystem:s.characterId==='runner'&&s.characterDefinition?.id==='runner'&&s.__characterSystemReady===true};probe.destroy();const ok=Object.values(checks).every(Boolean),detail=Object.entries(checks).map(([k,v])=>`${k}=${v?'ok':'FAIL'}`).join(' ');window.__WM_D1_SELF_TEST__={ok,...checks};document.documentElement.dataset.wreckmarchD1SelfTest=ok?'passed':'failed';window.__WM_LOG__?.(`D1 browser self-test ${ok?'PASSED':'FAILED'}: ${detail}`);if(!ok)throw Error('Phase D.1 self-test failed: '+detail)}
