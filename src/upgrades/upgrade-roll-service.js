...
function enforceGuaranteedRarity({ available, chosen, rolled, minimumRarity, rng }) {
  ...
  const replacementPool = available.filter(choice => !chosenIds.has(choice.id) && canSupportMinimumRarity(choice, minimumRarity));
 ...
 * @typedef {{ count?: number, rng?: () => number, excludeIds?: string[] | Set<string>, guaranteedMinimumRarity?: string | null }} UpgradeRollOptions
...
export function rollUpgradeChoices(choices, options = {}) {
  const { count = 3, rng = Math.random, excludeIds = [], guaranteedMinimumRarity = null } = options;
  ...
  const rarityGuarantee = guaranteedMinimumRarity == null ? null : normalizeUpgradeRarity(guaranteedMinimumRarity);
