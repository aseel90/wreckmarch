/* WRECKMARCH R3 — canonical semantic district/layout contract.
 * R2 technical sectors remain the streaming owner; this module is metadata only.
 */
...
const districtIdForSector = (column, row) => {
...
const landmark = (id, districtId, sectorKey, name, kind, localX, localY, scale = 1) =>
  Object.freeze({ id, districtId, sectorKey, name, kind, localX, localY, scale });
...
const clearing = (id, districtId, sectorKey, name, localX, localY, width, height) =>
  Object.freeze({ id, districtId, sectorKey, name, localX, localY, width, height, bossCapable: true, openGround: true });
...
export function getDistrictForSector(column, row) {
  ...
  if (!Number.isInteger(column) || !Number.isInteger(row) || column < 0 || row < 0 || column >= grid.columns || row >= grid.rows) {
    throw new RangeError(`Invalid production sector ${column}:${row}`);
  }
...
export function validateWorldDistrictContract() {
  ...
  if (CURRENT_PRODUCTION_WORLD.width !== 9600 || CURRENT_PRODUCTION_WORLD.height !== 9600) errors.push('R3 district contract requires the selected 9600x9600 R2 production world');
  if (WORLD_SECTOR_POLICY.sectorSize !== 1200 || !WORLD_SECTOR_POLICY.districtPartitionIndependent) errors.push('R3 districts must remain independent from the R2 1200-unit technical sector partition');

  for (let row = 0; row < grid.rows; row += 1) {
    for (let column = 0; column < grid.columns; column += 1) {
      const district = getDistrictForSector(column, row);
      if (!district) errors.push(`Production sector ${column}:${row} is uncovered`);
      else coverage.set(district.id, (coverage.get(district.id) || 0) + 1);
    }
  }

  for (const district of DISTRICT_DEFINITIONS) {
    if (!coverage.get(district.id)) errors.push(`${district.id} has no covered production sectors`);
  ...
  for (const item of WORLD_LANDMARKS) {
    ...
    const district = getDistrictForSector(column, row);
    if (district.id !== item.districtId) errors.push(`${item.id} landmark district does not match its sector`);
    if (item.localX < 80 || item.localX > 1120 || item.localY < 80 || item.localY > 1120) errors.push(`${item.id} landmark is outside safe sector bounds`);
  }

  const clearingIds = new Set();
  const clearingDistricts = new Set();
  for (const item of BOSS_CLEARINGS) {
    if (clearingIds.has(item.id)) errors.push(`Duplicate Boss clearing id ${item.id}`);
    clearingIds.add(item.id);
    clearingDistricts.add(item.districtId);
    const [column, row] = item.sectorKey.split(':').map(Number);
    const district = getDistrictForSector(column, row);
    if (district.id !== item.districtId) errors.push(`${item.id} Boss clearing district does not match its sector`);
    if (!item.bossCapable || !item.openGround || item.width < 480 || item.height < 420) errors.push(`${item.id} is not a valid Boss-capable open clearing`);
    if (item.localX - item.width / 2 < 80 || item.localX + item.width / 2 > 1120 || item.localY - item.height / 2 < 80 || item.localY + item.height / 2 > 1120) errors.push(`${item.id} Boss clearing exceeds safe sector bounds`);
  }
  if (clearingDistricts.size < 5) errors.push('Boss clearings must be distributed across at least five districts');

  if (getDistrictForSector(4, 4).id !== 'central-wreckroads') errors.push('Production start 4800,4800 must be inside Central Wreckroads');
...
if (!R3_WORLD_DISTRICT_VALIDATION.ok) throw new Error(`Invalid R3 district contract: ${R3_WORLD_DISTRICT_VALIDATION.errors.join('; ')}`);
