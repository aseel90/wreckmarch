...
export class WorldSectorTerrain {
  ...
  activateSector(sector) {
    ...
    if (!scene?.textures?.exists?.('c4-ground') || !scene?.textures?.exists?.('c4-road')) throw new Error('R2 world-sector terrain requires canonical c4-ground/c4-road textures');

    const objects = [];
    const districtMetadata = getDistrictMetadataForSector(sector);
    const { district, landmarks, bossClearings } = districtMetadata;
    ...
    const ground = markSectorObject(scene.add.tileSprite(centerX, centerY, width, height, 'c4-ground').setDepth(0.18).setName(`r2-sector-ground-${sector.key}`), sector.key, 'ground');
    ground.setTileScale(1);
    ground.tilePositionX = left;
    ground.tilePositionY = top;
    objects.push(ground);

    const wash = markSectorObject(scene.add.tileSprite(centerX, centerY, width, height, 'c4-ground').setDepth(0.24).setName(`r2-sector-wash-${sector.key}`).setAlpha(district.washAlpha).setTint(district.tint), sector.key, 'wash');
    ...
    for (const coordinate of roadCoordinates(this.world)) {
      if (intersectsBand(sector.x, sector.width, coordinate, roadHalf)) {
        const vertical = markSectorObject(scene.add.tileSprite(coordinate, centerY, ROAD_WIDTH, height, 'c4-road').setDepth(0.9).setName(`r2-sector-road-v-${coordinate}-${sector.key}`).setAlpha(1), sector.key, 'road');
      ...
      if (intersectsBand(sector.y, sector.height, coordinate, roadHalf)) {
        const horizontal = markSectorObject(scene.add.tileSprite(centerX, coordinate, width, ROAD_WIDTH, 'c4-road').setDepth(0.9).setName(`r2-sector-road-h-${coordinate}-${sector.key}`).setAlpha(1), sector.key, 'road');
    ...
    const insideBossClearing = (localX, localY) => bossClearings.some(clearing => Math.abs(localX - clearing.localX) < clearing.width / 2 + 70 && Math.abs(localY - clearing.localY) < clearing.height / 2 + 70);
    ...
    for (let index = 0; index < 5; index += 1) {
      ...
      if (!landmark && insideBossClearing(localX, localY)) {
      ...
      const object = markSectorObject(scene.add.ellipse(sector.x + localX, sector.y + localY, landmark ? (72 + rng() * 34) * scale : 24 + rng() * 60, landmark ? (34 + rng() * 24) * scale : 8 + rng() * 18, district.debrisTint, landmark ? 0.34 : 0.055 + rng() * 0.035).setDepth(landmark ? 1.18 : 1.12).setRotation(rng() * Math.PI), sector.key, landmark ? 'landmark' : 'debris');
  ...
  activeObjects(kind = null) {
    ...
    for (const sectorObjects of this.active.values()) {
      for (const object of sectorObjects) if (!kind || object?.__worldSectorTerrainKind === kind) objects.push(object);
    }
    return objects;
  }
  ...
  ensureVisible() {
    for (const object of this.activeObjects()) if (object?.active !== false) object?.setVisible?.(true);
  }
