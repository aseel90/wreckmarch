...
export class EnemyCombatSystem {
  ...
  hitByProjectile(bullet, enemy) {
    ...
    else if (ricochetRemaining > 0) bullet.ricochetPending = true;
    else bullet.destroy();
  ...
  spawnScrapRatHitFx(x, y, velocityX, velocityY) {
    ...
    const ring = scene.add.circle(x, y, 5, 0xf1b66f, .08)
      ...
      .setDepth(30);
    scene.tweens.add({ targets: ring, scale: 1.8, alpha: 0, duration: 95, onComplete: () => ring.destroy() });
    for (let i = 0; i < 3; i += 1) {
      ...
      scene.tweens.add({ targets: spark, x: x + Math.cos(angle) * distance, y: y + Math.sin(angle) * distance, alpha: 0, scale: .25, duration: Phaser.Math.Between(95, 125), onComplete: () => spark.destroy() });
    }
  }
  ...
  spawnSawbugHitFx(x, y, velocityX, velocityY) {
    const scene = this.scene;
    const impactAngle = Math.atan2(velocityY, velocityX) + Math.PI;
    const ring = scene.add.circle(x, y, 4.5, 0xc7d96f, .06).setStrokeStyle(1, 0xdce98d, .68).setDepth(30);
    scene.tweens.add({ targets: ring, scale: 1.7, alpha: 0, duration: 90, onComplete: () => ring.destroy() });
    for (let i = 0; i < 4; i += 1) {
      const droplet = scene.add.circle(x, y, i === 0 ? 1.9 : 1.25, i % 2 ? 0x70873b : 0xc6d76a, .88).setDepth(30);
      const angle = impactAngle + Phaser.Math.FloatBetween(-.65, .65);
      const distance = Phaser.Math.Between(7, 15);
      scene.tweens.add({ targets: droplet, x: x + Math.cos(angle) * distance, y: y + Math.sin(angle) * distance, alpha: 0, scale: .2, duration: Phaser.Math.Between(85, 120), onComplete: () => droplet.destroy() });
    }
  }
  ...
  spawnRustHoundHitFx(x, y, velocityX, velocityY) {
    const scene = this.scene;
    const impactAngle = Math.atan2(velocityY, velocityX) + Math.PI;
    const ring = scene.add.circle(x, y, 5.5, 0xc8794a, .06).setStrokeStyle(1, 0xe6a36f, .7).setDepth(30);
    scene.tweens.add({ targets: ring, scale: 1.65, alpha: 0, duration: 100, onComplete: () => ring.destroy() });
    for (let i = 0; i < 4; i += 1) {
      const spark = scene.add.circle(x, y, i < 2 ? 1.8 : 1.2, i % 2 ? 0x7b4636 : 0xe0a06b, .92).setDepth(30);
      const angle = impactAngle + Phaser.Math.FloatBetween(-.5, .5);
      const distance = Phaser.Math.Between(9, 18);
      scene.tweens.add({ targets: spark, x: x + Math.cos(angle) * distance, y: y + Math.sin(angle) * distance, alpha: 0, scale: .2, duration: Phaser.Math.Between(100, 135), onComplete: () => spark.destroy() });
    }
  }

  spawnScrapRatDeathFx(x, y, elite) { return this.spawnGenericDeathFx(x, y, elite, SCRAP_RAT_ID); }
  spawnSawbugDeathFx(x, y, elite) { return this.spawnGenericDeathFx(x, y, elite, SAWBUG_ID); }
  spawnRustHoundDeathFx(x, y, elite) { return this.spawnGenericDeathFx(x, y, elite, RUST_HOUND_ID); }

  spawnGenericDeathFx(x, y, elite, enemyId) {
    const scene = this.scene;
    const color = DEATH_BURST_COLORS[enemyId] ?? 0xc9784c;
    const count = elite ? 9 : 6;
    for (let i = 0; i < count; i += 1) {
      const shard = scene.add.circle(x, y, i % 3 === 0 ? 2.4 : 1.5, color, .9).setDepth(30);
      const angle = (Math.PI * 2 * i) / count + Phaser.Math.FloatBetween(-.2, .2);
      const distance = Phaser.Math.Between(elite ? 20 : 14, elite ? 34 : 26);
      scene.tweens.add({ targets: shard, x: x + Math.cos(angle) * distance, y: y + Math.sin(angle) * distance, alpha: 0, scale: .2, duration: Phaser.Math.Between(130, 200), onComplete: () => shard.destroy() });
    }
  }
  ...
  killEnemy(enemy) {
    ...
    const enemyId = enemy.enemyId;
    if (enemyId === SCRAP_RAT_ID) this.spawnScrapRatDeathFx(x, y, elite);
    else if (enemyId === SAWBUG_ID) this.spawnSawbugDeathFx(x, y, elite);
    else if (enemyId === RUST_HOUND_ID) this.spawnRustHoundDeathFx(x, y, elite);
    const dropCount = resolveEnemyScrapDropCount(enemy);
    enemy.destroy();
    for (let i = 0; i < dropCount; i += 1) this.scene.spawnScrap(x, y);
  }
}
