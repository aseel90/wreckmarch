...
function manualReportFailure(stage, error, extra = {}) {
  return { ok: false, stage, error: String(error?.message || error || 'unknown_error'), ...extra };
}
...
export async function sendCurrentRunReport(game = globalThis.__WM_GAME__, reason = 'MANUAL REPORT') {
  ...
  if (!provider) return manualReportFailure('transport', 'remote_reporting_unavailable');
  const telemetry = scene.runTelemetry;
  if (!telemetry) return manualReportFailure('session', 'run_telemetry_missing');
  let report;
  try { report = telemetry.finalized ? telemetry.getReport?.() : telemetry.finalize(reason); }
  catch (error) { return manualReportFailure('finalize', error); }
  ...
  try {
    ...
    if (stillQueued) return manualReportFailure('transport', status?.lastReportError || 'report_still_queued', { reportId, httpStatus: Number(status?.lastReportStatus) || 0, bytes: Number(status?.lastReportBytes) || 0, queueDepth: queue.length, flushResults });
    return { ok: true, stage: 'sent', reportId, httpStatus: Number(status?.lastReportStatus) || 0, bytes: Number(status?.lastReportBytes) || 0, queueDepth: queue.length, flushResults };
  } catch (error) {
    const status = globalThis.__WM_TELEMETRY_REMOTE_STATUS__ || null;
    return manualReportFailure('transport', error, { reportId, httpStatus: Number(status?.lastReportStatus) || 0, bytes: Number(status?.lastReportBytes) || 0, queueDepth: provider.getQueue().length });
  }
}
