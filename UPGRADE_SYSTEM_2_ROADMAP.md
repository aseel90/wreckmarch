...
- [x] Define upgrade-level snapshot format. — **Status:** ✅ DONE
- [x] Define stat-modifier snapshot format. — **Status:** ✅ DONE
- [x] Define mechanical-effect snapshot format. — **Status:** ✅ DONE
- [x] Add round-trip tests before any persistence feature uses it. — **Status:** ✅ DONE
  - Verified on `a53dc9e`: `wreckmarch.upgrade-run-state` schema v1 snapshots upgrade levels + rarity history, Character/Weapon stat modifiers and caps, and persistent mechanical state. Restore targets must be initialized and clean; Twin Riveter state is restored directly, Call the Rig restores through canonical `RigSystem.summon()`, and acquisition-only `RESTORE_HP` is intentionally excluded so Armor Plate cannot heal twice on load. Unit round-trip/validation and Chromium round-trip passed; PR #99 Quality, Smoke, all three E2E shards and aggregate E2E passed, then Pages/iOS live verification succeeded on exact SHA `a53dc9e3e2f9d4874c4290cb3704ad7cafabc25e` with no CI/Live failure issue opened during the production run window.
  - Scope remains readiness only: no `localStorage`, save slots, automatic persistence/load, meta-progression, or Phaser-object serialization was added.
